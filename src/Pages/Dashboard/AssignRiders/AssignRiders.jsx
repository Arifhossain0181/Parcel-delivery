...
const AssignRiders = () => {
  ...
  // Fetch riders by district (or division)
  const fetchRidersByDistrict = async (district) => {
    try {
      setLoadingRiders(true);
      const res = await axiosSecure.get(
        `/rideres/by-district?district=${district}`
      );
      setRiders(res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleAssignRider = async (riderId, riderEmail) => {
    ...
    try {
      ...
      // Update parcel deliveryStatus to "In Transit"
      await axiosSecure.patch(`/parcels/update-status/${selectedParcel._id}`, {
        status: "in_transit",
      });

      if (res.data.parcelUpdated || res.data.riderUpdated) {
        Swal.fire(" Success", "Rider assigned and parcel in transit!", "success");
        setSelectedParcel(null);
        refetch();
      } else {
        Swal.fire("⚠️ Warning", "No changes were made!", "warning");
      }
    } catch (error) {
      console.error(error);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-11/12 sm:w-2/3 lg:w-1/2 max-h-[80vh] overflow-y-auto">
            ...
            {loadingRiders ? (
              ...
              <ul className="space-y-2">
                {riders.map((rider) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        handleAssignRider(rider._id, rider.email)
                      }
              ...
              <p className="text-center text-gray-500">No active riders in this district.</p>
            )}
